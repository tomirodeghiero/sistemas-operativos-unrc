...
s_trap:
    # push cpu registers in same order as defined in struct trap_frame
    addi    sp, sp, -30 * 4
    sw      ra, 0 * 4 (sp)
    sw      gp, 1 * 4 (sp)
    sw      t0, 2 * 4 (sp)
...
m_trap:
    # save a0-a7 registers.
    # Some of them are used here and in next_timer_interrupt() function
    addi    sp, sp, -8 * 4
    sw      a0, 0 * 4 (sp)
    sw      a1, 1 * 4 (sp)
    sw      a2, 2 * 4 (sp)
    ...
    # schedule the next timer interrupt: next_timer_interrupt(cpu_id)
    mv      a0, tp
    call    next_timer_interrupt
    ...
    # set timer interrupt pending bit in sip to delegate interrupt to S-mode
    li a1, 2
    csrw sip, a1

    # CPU is configured to delegate all interrupts to S-mode
    # mret will jump to s-trap as a software interrupt (scause = 0x80000001)
    mret
